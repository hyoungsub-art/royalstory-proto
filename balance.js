...
window.BALANCE = {
  ...
  ECON: {
    mineCycle: 14,    // 금광 채굴 주기(초) — 주기마다 금광 레벨만큼 골드 (2026-09-15: 2.8→14, 채굴 5배 느리게)
    mineMax: 5,       // 금광 최대 레벨
          regen: 2.8, start: 4, cap: [0, 10, 20, 30], gasBonus: 0.5,   // start(2026-09-15) = 금광 1채 건설비. regen·gasBonus는 레거시 (자연 회복 폐지)
          ...
          tierGold: [0, 0, 40, 100],     // 승급 비용(골드): 킵 40 / 캐슬 100 (2026-09-13 대폭 인상, 가정)
  ...
  BUILD_RING: [0, 4, 6, 8],    // 거점 주변 보조 띠 (본진 지역과 합집합)   // 건설 띠 폭[티어] (2026-09-12 확정): 본진·거점 풋프린트 주변 — 타운홀 2 / 킵 4 / 캐슬 6 (승급마다 +2, 강등 없음). 거점 자체는 어디든 건설 가능
  ...
  FOG_ON: 1,        // 전장의 안개 사용 (2026-09-15, 렌더 전용)
  VISION: { unit: 4, margin: 1.5, hall: 5, tower: 5, mine: 3, bldg: 3.5 },   // 시야 반경(타일) — 유닛은 max(unit, 사거리+margin)
  PROD_CAP: 5,      // 생산 건물당 살아 있는 자기 유닛 상한 (2026-09-13) — 미만이면 재생산, 대기 중에도 게이지는 채워 둠
  ...
  STANCE: { holdR: 2, aggroR: 3, leash: 4 },   // aggroR(2026-09-15): 적 인지 범위 6→3칸 — 유지 반경 ×1.4 / leash: 추격 시작점에서 이 칸수 넘으면 교전 해제 — 끝없이 딸려가기 방지 (가정)
  ...
  UNITS: {
    ...
    mage:          { hp: 620,  dmg: 130, hs: 1.4, rng: 2.6, spd: 1.0, splash: 1.8 },   // 2026-09-15 하향: 사거리 3.5→2.6, 공격력 190→130
  },
